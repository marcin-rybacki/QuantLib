...
namespace overnight_indexed_swap_test {
    ...
    struct CommonVars {
        ...
        makeSwap(Period length,
            ...
            return MakeOIS(length, eoniaIndex, fixedRate, 0 * Days, subPeriodsNettingType)
...
namespace overnight_indexed_swap_test {
    ...
    for (Size i = 0; i < LENGTH(eoniaSwapData); i++) {
        ...
        ext::shared_ptr<OvernightIndexedSwap> swap =
            vars.makeSwap(term, 0.0, 0.0, false, Null<Date>(), paymentLag, subPeriodsNettingType);
...
void OvernightIndexedSwapTest::testBootstrap() {
    ...
    overnight_indexed_swap_test::testBootstrap(false, OvernightCouponNettingType::Compounding);
}
...
void OvernightIndexedSwapTest::testBootstrapWithArithmeticAverage() {
    ...
    overnight_indexed_swap_test::testBootstrap(false, OvernightCouponNettingType::Averaging);
}
...
void OvernightIndexedSwapTest::testBootstrapWithTelescopicDates() {
    ...
    overnight_indexed_swap_test::testBootstrap(true, OvernightCouponNettingType::Compounding);
}
...
void OvernightIndexedSwapTest::testBootstrapWithTelescopicDatesAndArithmeticAverage() {
    ...
    // is needed.
    overnight_indexed_swap_test::testBootstrap(true, OvernightCouponNettingType::Averaging, 1.0e-5);
}
